...
class AttnWrapper(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        if self.act_as_identity:
            # print(kwargs)
            kwargs['attention_mask'] += kwargs['attention_mask'][0, 0, 0, 1] * torch.tril(
                torch.ones(kwargs['attention_mask'].shape,
...
class BlockOutputWrapper(torch.nn.Module):
    ...
    def forward(self, *args, **kwargs):
        ...
        mlp_output = self.block.mlp(
            self.post_attention_layernorm(attn_output.to(self.post_attention_layernorm.weight.device)))
